...
function enhanceCard(card){
  ...
  let last=hex.value;const sync=()=>{const next=normalize(picker.value)||last||'FFFFFF';last=next;if(document.activeElement!==hex||hex.value.length===6)hex.value=next},apply=()=>{const clean=normalize(hex.value);hex.value=clean;if(clean.length!==6)return false;last=clean;const value='#'+clean;if(picker.value.toUpperCase()!==value){picker.value=value;picker.dispatchEvent(new Event('input',{bubbles:true}))}return true};
...
function enhanceAll(){list.querySelectorAll('.text-card').forEach(enhanceCard)}enhanceAll();document.addEventListener('fw:text-ui-rendered',enhanceAll);
})();
