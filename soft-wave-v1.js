...
function spectrum(q,r){
  const bins=r?.bins;if(bins?.length){const max=Math.min(bins.length-1,430),u=Math.pow(clamp(q,0,1),1.42),center=Math.round(2+u*(max-2)),rad=3;let sum=0,weight=0;for(let d=-rad;d<=rad;d++){const i=clamp(center+d,0,bins.length-1),w=rad+1-Math.abs(d);sum+=bins[i]*w;weight+=w}return weight?sum/weight/255:0}
  const bass=r?.bass||0,mid=r?.mid||0,treble=r?.treble||0;return clamp(bass*Math.pow(1-q,.9)+mid*Math.sin(Math.PI*q)*.72+treble*Math.pow(q,.7)*.55,0,1.3)
}
function barSource(style,q,r){if(style==='mirrorBars'||style==='centerBars'){const m=Math.abs(q-.5)*2;return spectrum(m,r)}return spectrum(q,r)}
...
function heightsFor(w,r,count){
  let arr=smoothState.get(w.id);if(!arr||arr.length!==count){arr=Array(count).fill(.12);smoothState.set(w.id,arr)}
  const smoothing=clamp((w.smoothing||0)/95,0,1),attack=.34-(smoothing*.22),release=.20-(smoothing*.13),beat=(r?.beat||0)*(w.beatPunch/100)*.10,reaction=(w.reaction||0)/100;
  for(let i=0;i<count;i++){
    const q=count<=1?.5:i/(count-1),src=barSource(w.style,q,r),micro=.84+.16*Math.sin((i+1)*1.71+(r?.time||0)*.62),target=clamp(.10+Math.pow(src,1.12)*reaction*.88*micro+beat,0,1.18),a=target>arr[i]?attack:release;arr[i]+= (target-arr[i])*a;
  }
  return arr;
}
function softBounds(w){const W=canvas.width,H=canvas.height,cx=W*(w.x||50)/100,cy=H*(w.y||50)/100,width=W*(w.size||54)/100,maxH=H*(.022+clamp((w.toothDepth||60)/220,0,1)*.105);return{x:cx-width/2,y:cy-maxH*.72,w:width,h:maxH*1.44,cx,cy,maxH}}
function drawLayer(w,r){
  ...
  const rounded=w.style==='roundedBars'?1:w.style==='centerBars'?.7:w.style==='mirrorBars'?.42:Math.max(.08,1-(w.sharpness||50)/100),maxH=b.maxH,minH=canvas.height*.0065;
...
function syncUI(){
  ...
  if(soft){if(shape)shape.disabled=true;if(maxSize)maxSize.disabled=true;if(hint)hint.textContent='Soft soundwave: drag anywhere on the bars. Detail = bar count · Thickness = bar width · Tooth depth = max height · Smoothing = calmness.'}
