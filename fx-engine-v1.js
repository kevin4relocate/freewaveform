...
const TYPES={
  ...
  stars:{name:'Stars',icon:'★',amount:48,speed:12,size:30,opacity:48,intensity:135,color:'#fff6da',reaction:'ambient',strength:34,blend:'screen'},
  rays:{name:'Light Rays',icon:'☀',amount:34,speed:16,size:112,opacity:34,intensity:145,color:'#f5d69b',reaction:'beat',strength:42,blend:'screen'},
  club:{name:'Club Bars',icon:'▥',amount:40,speed:80,size:76,opacity:42,intensity:140,color:'#d45cff',reaction:'beat',strength:76,blend:'lighter'}
};
...
function mount(){
  ...
  root=document.createElement('div');root.innerHTML=`
    <div class="card fx-card">
      <div class="card-title"><div><strong>FX Templates</strong><small>Clearer atmospheric effects behind waveform + text</small></div><span class="badge fx-quality-badge">PERFORMANCE</span></div>
      <div class="fx-quality-row">
        ...
        <p class="hint fx-quality-note">Performance keeps the editor light. Export automatically uses Full quality.</p>
      </div>
    ...
    <div class="card fx-settings" hidden>
      ...
      <div class="fx-color-row"><input class="fx-color" type="color" value="#ffffff"><small>Color stays vivid while Intensity controls how strongly the FX separates from the background.</small></div>
      ...
      <p class="hint fx-note">Start around 110–150% Intensity. Use 1–3 layers for a clean result.</p>
    </div>`;
...
function drawStars(l,r,t){
  const n=countFor(l),p=pulse(l,r),base=px(1+l.size*.045),alpha=alphaFor(l);
  for(let i=0;i<n;i++){
    const x=rnd(l.seed,i*5+1)*drawCanvas.width,y=rnd(l.seed,i*5+2)*drawCanvas.height;
    const tw=.45+.55*Math.abs(Math.sin(t*(.5+l.speed*.018)*(.45+rnd(l.seed,i*5+3))+rnd(l.seed,i)*12));
    const a=alpha*tw*(1+p*.75),s=base*(.45+rnd(l.seed,i*5+4)*1.25)*(1+p*.14);
    drawCtx.fillStyle=rgba(l.color,a);drawCtx.shadowColor=rgba(l.color,a);drawCtx.shadowBlur=s*3.8*blurScale();drawCtx.beginPath();drawCtx.arc(x,y,s,0,Math.PI*2);drawCtx.fill();
    if(i%5===0){const arm=s*(2.2+rnd(l.seed,i*5+5)*1.8);drawCtx.strokeStyle=rgba(l.color,a*.72);drawCtx.lineWidth=Math.max(px(.6),s*.22);drawCtx.beginPath();drawCtx.moveTo(x-arm,y);drawCtx.lineTo(x+arm,y);drawCtx.moveTo(x,y-arm);drawCtx.lineTo(x,y+arm);drawCtx.stroke()}
  }
}
...
function drawRays(l,r,t){
  ...
  for(let i=0;i<n;i++){
    ...
    const core=.34,ix1=ox+(x1-ox)*(1-core),ix2=ox+(x2-ox)*(1-core);drawCtx.fillStyle=rgba('#fff7dd',alpha*.10);drawCtx.beginPath();drawCtx.moveTo(ox,oy);drawCtx.lineTo(ix1,drawCanvas.height*.82);drawCtx.lineTo(ix2,drawCanvas.height*.82);drawCtx.closePath();drawCtx.fill();
  }
}
function drawClub(l,r,t){
  ...
  for(let i=0;i<n;i++){
    ...
    if(i%3===0){drawCtx.strokeStyle=rgba('#ffffff',a*.34);drawCtx.shadowBlur=0;drawCtx.lineWidth=Math.max(px(.6),w*.14);drawCtx.beginPath();drawCtx.moveTo(x1,y1);drawCtx.lineTo(x2,y2);drawCtx.stroke()}
  }
  if(p>.12){drawCtx.fillStyle=rgba(l.color,clamp(alpha*p*.085,0,.12));drawCtx.fillRect(0,0,drawCanvas.width,drawCanvas.height)}
}
