...
function normalizeLayer(w={},index=0,{newId=false}={}){
  ...
  return{
    ...
    size:n(w.size,DEFAULT_WAVE.size,12,85),thickness:n(w.thickness,DEFAULT_WAVE.thickness,1,16),opacity:n(w.opacity,DEFAULT_WAVE.opacity,5,100),detail:n(w.detail,DEFAULT_WAVE.detail,16,256),toothDepth:n(w.toothDepth,DEFAULT_WAVE.toothDepth,0,220),sharpness:n(w.sharpness,DEFAULT_WAVE.sharpness,0,100),
...
document.addEventListener('input',e=>{
  ...
  if(rangeMeta[id]){const [key,suf,out]=rangeMeta[id];w[key]=+e.target.value;if(el(out))el(out).textContent=e.target.value+suf;if(key==='smoothing')window.__FW_APP?.updateAnalyserSettings?.();document.dispatchEvent(new CustomEvent('fw:wave-setting-change',{detail:{key,value:w[key]}}))}
