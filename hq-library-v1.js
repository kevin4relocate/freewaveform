...
async function apiFetch(url,options={},retry=true){
  ...
  if(res.status===401&&retry){const entered=prompt('Enter your HQ Export Key');if(entered){localStorage.setItem('fw.hqExportKey',entered.trim());return apiFetch(url,options,false)}}
  return res;
}
function styleMount(){
...
function render(totalBytes=0){
  ...
  const bulk=modal.querySelector('[data-hq-delete-finished]');if(bulk)bulk.disabled=!finished.length;
}
async function load(){
...
function open(){ensureModal().hidden=false;load()}
...
window.__FW_HQ_LIBRARY={open,close,refresh:load};
})();
