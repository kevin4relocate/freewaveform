...
function notify(text){const el=$('toast');if(!el)return;el.textContent=text;el.classList.add('show');clearTimeout(notify.t);notify.t=setTimeout(()=>el.classList.remove('show'),2800)}
...
function sanitizeValues(values){
  ...
  const clean={};
  Object.entries(values).forEach(([key,value])=>{if(allowedIds.has(key)&&(typeof value==='string'||typeof value==='number'||typeof value==='boolean'))clean[key]=value});
...
function captureWaveDom(base={}){
  const out={...base};
  out.style=$('waveStyle')?.value||out.style||'brushRing';out.shape=$('waveShape')?.value||out.shape||'circle';
  Object.entries(waveRangeMap).forEach(([id,key])=>{const el=$(id);if(el)out[key]=+el.value});
  out.color=$('waveColor')?.value||out.color||'#e5d3a6';
  ['showWave','showPlate','showGlow','showSecondary'].forEach(id=>{const el=$(id);if(el)out[id]=!!el.checked});
  return out;
}
function captureWaveLayers(){
  const api=window.__FW_MULTI_WAVE;
  let layers=api?.getLayers?.()||[];
  const active=Number(api?.getActiveIndex?.()||0);
  if(layers.length){layers=layers.map(x=>({...x}));layers[active]=captureWaveDom(layers[active]);return{layers,activeIndex:active}}
...
function sanitizeWaveLayer(w,index){
  ...
  return{
    name:String(w.name||`Wave ${index+1}`).slice(0,30),
    style:waveStyles.has(w.style)?w.style:'brushRing',shape:waveShapes.has(w.shape)?w.shape:'circle',
    reaction:n(w.reaction,130,0,400),beatPunch:n(w.beatPunch,150,0,300),beatSensitivity:n(w.beatSensitivity,135,50,250),smoothing:n(w.smoothing,55,0,95),glow:n(w.glow,16,0,100),
    size:n(w.size,46,12,85),thickness:n(w.thickness,4,1,16),opacity:n(w.opacity,78,5,100),detail:n(w.detail,128,16,256),toothDepth:n(w.toothDepth,80,0,220),sharpness:n(w.sharpness,70,0,100),
    color:validColor(w.color,'#e5d3a6'),showWave:w.showWave!==false,showPlate:!!w.showPlate,showGlow:w.showGlow!==false,showSecondary:!!w.showSecondary,
    x:n(w.x,50,3,97),y:n(w.y,50,3,97)
  };
}

function captureTexts(){
  const cards=[...document.querySelectorAll('#textList .text-card')];
  const positions=window.__FW_TEXT_BRIDGE?.getPositions?.()||[];
  return cards.map((card,i)=>{
    const get=k=>card.querySelector(`[data-k="${k}"]`);
    const p=positions[i]||{};
    return{
      label:String(card.querySelector('header strong')?.textContent||`Text ${i+1}`),
      text:String(get('text')?.value||''),font:String(get('font')?.value||'sans'),color:validColor(get('color')?.value,'#ffffff'),
      size:+get('size')?.value||42,opacity:+get('opacity')?.value||100,show:!!get('show')?.checked,
      react:String(get('react')?.value||'false')==='true',strength:+get('strength')?.value||0,
      x:Number.isFinite(p.x)?p.x:undefined,y:Number.isFinite(p.y)?p.y:undefined
    };
  });
}
function sanitizeText(t,index){
  ...
  const n=(v,d,a,b)=>clamp(Number.isFinite(+v)?+v:d,a,b);
  const clean={
    label:String(t.label||`Text ${index+1}`).slice(0,50),text:String(t.text??'').slice(0,500),font:String(t.font||'sans').slice(0,64),color:validColor(t.color,'#ffffff'),
    size:n(t.size,42,14,160),opacity:n(t.opacity,100,5,100),show:t.show!==false,react:!!t.react,strength:n(t.strength,0,0,80)
  };
  if(Number.isFinite(+t.x))clean.x=n(t.x,50,2,98);if(Number.isFinite(+t.y))clean.y=n(t.y,50,2,98);
  return clean;
}
...
function captureProject(){
  ...
  return{
    schema:2,
    ratio:document.querySelector('[data-ratio].active')?.dataset.ratio||'16:9',
    controls:captureControls(),
    waveLayers:wave.layers,
    waveActiveIndex:wave.activeIndex,
    texts:captureTexts()
  };
}
function sanitizeProject(p){
  if(!p||typeof p!=='object'||Array.isArray(p))return null;
  const controls=sanitizeValues(p.controls||p.values||{})||{};
  const waveLayers=Array.isArray(p.waveLayers)?p.waveLayers.slice(0,5).map(sanitizeWaveLayer).filter(Boolean):[];
  const texts=Array.isArray(p.texts)?p.texts.slice(0,10).map(sanitizeText).filter(Boolean):[];
  return{schema:2,ratio:p.ratio==='9:16'?'9:16':'16:9',controls,waveLayers,waveActiveIndex:clamp(+p.waveActiveIndex||0,0,Math.max(0,waveLayers.length-1)),texts};
}
...
function applyWaveLayer(w){
  setWaveControl('waveStyle',w.style,'change');setWaveControl('waveShape',w.shape,'change');
  Object.entries(waveRangeMap).forEach(([id,key])=>setWaveControl(id,w[key],'input'));
  setWaveControl('waveColor',w.color,'input');
  ['showWave','showPlate','showGlow','showSecondary'].forEach(id=>setWaveControl(id,w[id],'change'));
}
function placeActiveWave(w){
  if(EDGE.has(w.style)||!Number.isFinite(w.x)||!Number.isFinite(w.y))return;
  const canvas=$('canvas');if(!canvas)return;
  const r=canvas.getBoundingClientRect(),E=window.PointerEvent||window.MouseEvent;
  const clientX=r.left+r.width*w.x/100,clientY=r.top+r.height*w.y/100,common={bubbles:true,cancelable:true,clientX,clientY,button:0,pointerId:91,isPrimary:true,pointerType:'mouse'};
...
function restoreWaveLayers(project){
  ...
  const root=document.querySelector('.multi-wave-card');
  ...
  layers.forEach((w,i)=>{
    if(i>0)root.querySelector('.mw-add')?.click();
    applyWaveLayer(w);placeActiveWave(w);
  });
  const items=[...root.querySelectorAll('.mw-item .mw-select')];items[clamp(project.waveActiveIndex||0,0,items.length-1)]?.click();
  if(prev&&prev!=='waveform')document.querySelector(`.tool[data-tool="${prev}"]`)?.click();
}
function ensureTextCount(count){
  count=clamp(count,2,10);
  let cards=()=>[...document.querySelectorAll('#textList .text-card')];
  while(cards().length>count){const del=cards().at(-1)?.querySelector('.delete-text');if(!del)break;del.click()}
  while(cards().length<count){$('addText')?.click();if(cards().length>=10)break}
}
...
async function restoreTexts(texts){
  if(!Array.isArray(texts)||!texts.length)return;
  ensureTextCount(texts.length);window.__FW_TEXT_BRIDGE?.enhanceAll?.();
  const cards=[...document.querySelectorAll('#textList .text-card')];
  texts.forEach((t,i)=>{const c=cards[i];if(!c)return;setTextField(c,'show',t.show);setTextField(c,'text',t.text);setTextField(c,'font',t.font);setTextField(c,'color',t.color);setTextField(c,'size',t.size);setTextField(c,'opacity',t.opacity);setTextField(c,'react',String(!!t.react));setTextField(c,'strength',t.strength)});
  await twoFrames();
  texts.forEach((t,i)=>{if(Number.isFinite(t.x)&&Number.isFinite(t.y))window.__FW_TEXT_BRIDGE?.moveTextTo?.(i,t.x,t.y)});
}
async function applyProject(project,legacyValues){
  if(!project){applyControls(legacyValues);return}
  const ratioBtn=document.querySelector(`[data-ratio="${project.ratio}"]`);ratioBtn?.click();
  ['reactScope','syncMode'].forEach(id=>{if(project.controls?.[id]!==undefined)setWaveControl(id,project.controls[id],'change')});
  if(project.controls?.scenePunch!==undefined)setWaveControl('scenePunch',project.controls.scenePunch,'input');
  restoreWaveLayers(project);
  await restoreTexts(project.texts);
}
...
function render(){
  ...
  if(!items.length){list.innerHTML='<div class="user-preset-empty">No saved presets yet.</div>';return}
...
function saveCurrent(){
  const input=root.querySelector('.user-preset-name');let name=input.value.trim(),items=loadAll();if(!name)name='My Preset '+(items.length+1);const now=Date.now(),project=captureProject(),values=project.controls;
  const existing=items.find(x=>String(x.name).toLowerCase()===name.toLowerCase());
  if(existing){existing.values=values;existing.project=project;existing.updatedAt=now;saveAll(items);notify('Updated full preset: '+name)}
  else{items.unshift({id:makeId(),name,values,project,createdAt:now,updatedAt:now});saveAll(items);notify('Saved full preset: '+name)}
  input.value='';render();
}
function exportPresets(){
  const items=loadAll();if(!items.length){notify('No presets to export');return}
  const payload={app:'FreeWaveform',type:'project-presets',version:FILE_VERSION,exportedAt:new Date().toISOString(),presets:items};
  const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='freewaveform-presets-'+safeFileStamp()+'.json';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);notify('Exported '+items.length+' full preset'+(items.length===1?'':'s'));
}
...
function mount(){
  const panel=document.querySelector('[data-panel="waveform"]'),templateGrid=$('templateGrid');if(!panel||!templateGrid||document.querySelector('.user-presets-card'))return false;const templateCard=templateGrid.closest('.card');if(!templateCard)return false;
  root=document.createElement('div');root.className='card user-presets-card';root.innerHTML=`<div class="card-title"><div><strong>My Presets</strong><small>Wave layers + text styles + positions</small></div><span class="badge user-preset-count">0 / ${MAX_PRESETS}</span></div><div class="user-preset-save-row"><input class="user-preset-name" type="text" maxlength="40" placeholder="Preset name, e.g. Kung Fu Punch" /><button class="button accent user-preset-save" type="button">Save current</button></div><div class="user-preset-transfer-row"><button class="button user-preset-export" type="button">⇩ Export presets</button><button class="button user-preset-import" type="button">⇧ Import presets</button><input class="user-preset-file" type="file" accept="application/json,.json" hidden /></div><div class="user-preset-list"></div><p class="hint">New presets save all waveform layers plus text content, font, color, size, opacity, pulse settings and positions. Export/import moves them between browsers. Older waveform-only preset files still work.</p>`;templateCard.insertAdjacentElement('afterend',root);
  const style=document.createElement('style');style.textContent=`.user-presets-card{margin-top:12px}.user-preset-save-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:center}.user-preset-transfer-row{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:8px}.user-preset-transfer-row .button{height:34px;padding:0 10px;font-size:9px}.user-preset-name{width:100%;height:38px;padding:0 11px;border:1px solid #2b3236;border-radius:9px;background:#0a0f11;color:#eee6da;outline:none;font:500 10px Inter,system-ui,sans-serif}.user-preset-name:focus{border-color:#a87943;box-shadow:0 0 0 2px rgba(210,161,91,.10)}.user-preset-save{height:38px;white-space:nowrap;padding:0 12px}.user-preset-list{display:grid;gap:6px;margin-top:9px}.user-preset-item{display:grid;grid-template-columns:minmax(0,1fr) 34px;gap:6px;align-items:stretch}.user-preset-load{min-width:0;height:42px;border:1px solid #293034;border-radius:9px;background:#0c1113;color:#d8d0c4;display:flex;align-items:center;gap:9px;padding:0 10px;text-align:left;cursor:pointer}.user-preset-load:hover{border-color:#7d5c36;background:#15130f}.user-preset-load>span:last-child{min-width:0;display:flex;align-items:center;gap:8px;flex:1}.user-preset-load b{font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:1}.user-preset-load small{font-size:8px;color:#777;white-space:nowrap}.user-preset-icon{width:22px;height:22px;border-radius:7px;display:grid;place-items:center;background:#1b1710;color:#d9a95f;border:1px solid #5b4327;font-size:10px;flex:none}.user-preset-delete{height:42px;border:1px solid #293034;border-radius:9px;background:#0c1113;color:#777;cursor:pointer;font-size:16px}.user-preset-delete:hover{color:#f1b8a7;border-color:#71443a;background:#1b1110}.user-preset-empty{padding:9px 10px;border:1px dashed #293034;border-radius:9px;color:#777;font-size:9px;text-align:center}@media(max-width:520px){.user-preset-save-row,.user-preset-transfer-row{grid-template-columns:1fr}.user-preset-save{width:100%}}`;document.head.appendChild(style);
