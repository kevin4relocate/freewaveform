...
function sanitizeText(t={},index=0){
  const out={label:String(t.label||`Text ${index+1}`).slice(0,50),text:String(t.text??'').slice(0,500),font:String(t.font||'sans').slice(0,64),color:validColor(t.color,'#ffffff'),size:num(t.size,42,14,160),opacity:num(t.opacity,100,5,100),show:t.show!==false,react:!!t.react,strength:num(t.strength,0,0,80)};
...
function ensureTextCount(count){count=clamp(count,2,10);const cards=()=>[...document.querySelectorAll('#textList .text-card')];while(cards().length>count){const del=cards().at(-1)?.querySelector('.delete-text');if(!del)break;del.click()}while(cards().length<count){$('addText')?.click();if(cards().length>=10)break}}
